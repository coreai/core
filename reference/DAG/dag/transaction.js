...
const {votingReward} = require('../config')
...
function checkTransaction (transaction, unspent) {
  ...
  if (transaction.reward) {
    // For reward block check if amount is correct
    if (transaction.outputs.length !== 1) throw new TransactionError('Reward transaction must have exctly one output')
    if (transaction.outputs[0].amount !== votingReward) throw new TransactionError(`Voting reward must be exactly: ${votingReward}`)
  } else {
...
/**
 * Create reward transaction for block voting
 *
 * @param address
 * @return {{id: string, reward: boolean, inputs: *, outputs: *, hash: string}}
 */
function createRewardTransaction (address) {
  return createTransaction([], [{index: 0, amount: votingReward, address}], true)
}
...
/**
 * Build a transaction to send data
 *
 * @param wallet
 * @param toAddress
 * @param amount
